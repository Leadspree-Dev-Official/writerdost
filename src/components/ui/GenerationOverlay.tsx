    ...
    return (
      <AnimatePresence>
        <motion.div 
          ...
          className="fixed bottom-6 right-6 z-[100] bg-emerald-500 text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-4 cursor-pointer hover:bg-emerald-600 transition-colors border border-emerald-400"
          onClick={() => {
             setLocalFinished(false);
             setMinimized(false);
          }}
        >
          <span className="material-symbols-outlined text-3xl animate-bounce">task_alt</span>
          <div>
             <p className="text-base font-black tracking-wide">Task Completed!</p>
    ...
    return (
      <AnimatePresence>
        <motion.div
          ...
          animate={{ opacity: 1, y: 0, scale: 1 }}
          exit={{ opacity: 0, scale: 0.9 }}
          className="fixed bottom-6 right-6 z-[100] bg-white dark:bg-[#141420] text-slate-900 dark:text-white p-4 rounded-2xl shadow-[0_20px_50px_-10px_rgba(0,0,0,0.2)] dark:shadow-[0_20px_50px_-10px_rgba(0,0,0,0.6)] flex items-center gap-4 cursor-pointer hover:bg-slate-50 dark:hover:bg-[#1c1c2e] transition-colors group border border-slate-200 dark:border-white/[0.06] min-w-[300px]"
          onClick={() => setMinimized(false)}
        >
          <div className="w-10 h-10 rounded-xl bg-slate-900 dark:bg-primary flex items-center justify-center shrink-0">
          ...
          <div className="flex-1 min-w-0">
             ...
             <div className="flex justify-between items-end mt-1 mb-1">
               ...
               <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">{elapsed}</span>
             </div>
    ...
    <AnimatePresence>
      ...
      >
        <motion.div 
          ...
          className="w-full max-w-4xl bg-white dark:bg-[#0c0c14] border border-slate-100 dark:border-white/[0.06] shadow-[0_40px_120px_-20px_rgba(0,0,0,0.3)] dark:shadow-[0_40px_120px_-20px_rgba(0,0,0,0.7)] rounded-[2.5rem] overflow-hidden flex flex-col max-h-[85vh] relative m-4"
        >
          ...
          <div className="px-10 py-8 border-b border-slate-50 dark:border-white/[0.04] flex items-center justify-between bg-slate-50/50 dark:bg-white/[0.02]">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 bg-slate-900 dark:bg-primary rounded-2xl flex items-center justify-center shadow-xl shadow-slate-900/20 dark:shadow-primary/20">
                <span className="material-symbols-outlined text-white text-2xl animate-spin-slow">settings</span>
              </div>
              <div>
                <h2 className="text-xl font-black text-slate-900 dark:text-white tracking-tight leading-none mb-1">{title}</h2>
                <p className="text-[10px] text-slate-400 dark:text-slate-500 font-black uppercase tracking-[0.2em]">{subtitle}</p>
              </div>
            </div>
            
            <div className="flex items-center gap-3">
               <div className="px-4 py-2 bg-white dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.06] rounded-xl flex flex-col justify-center shadow-sm dark:shadow-none">
                  <span className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">API Status</span>
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                    <span className="text-[11px] font-black text-slate-700 dark:text-slate-300 leading-none">Live</span>
                  </div>
               </div>
               <div className="px-4 py-2 bg-white dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.06] rounded-xl flex flex-col justify-center shadow-sm dark:shadow-none min-w-[80px]">
                  <span className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Tokens Used</span>
                  <span className="text-[11px] font-black text-slate-700 dark:text-slate-300 leading-none mt-0.5">{(sessionTokens || 0).toLocaleString()}</span>
               </div>
               <div className="px-4 py-2 bg-slate-900 dark:bg-primary rounded-xl text-white flex items-center gap-2 shadow-lg dark:shadow-primary/20 h-full">
                  <span className="material-symbols-outlined text-sm">timer</span>
                  <span className="text-xs font-black tabular-nums">{elapsed}</span>
               </div>
               <button 
                  onClick={() => setMinimized(true)}
                  className="w-10 h-10 flex items-center justify-center bg-white dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.06] text-slate-600 dark:text-slate-400 rounded-xl hover:bg-slate-50 dark:hover:bg-white/[0.08] hover:text-slate-900 dark:hover:text-white transition-colors shadow-sm dark:shadow-none ml-2"
          ...
          <div className="flex-1 overflow-hidden grid grid-cols-12 gap-0">
            ...
            <div className="col-span-12 lg:col-span-5 p-10 flex flex-col border-r border-slate-50 dark:border-white/[0.04] bg-white dark:bg-transparent relative">
               <div className="mb-8 p-8 bg-slate-50 dark:bg-white/[0.03] rounded-[2rem] border border-slate-100/50 dark:border-white/[0.06] flex flex-col items-center text-center relative overflow-hidden group shadow-inner dark:shadow-none">
                  ...
                  <div className="relative mb-6">
                    <div className="w-24 h-24 rounded-3xl bg-white dark:bg-white/[0.06] flex items-center justify-center shadow-lg dark:shadow-none relative z-10 border border-slate-100 dark:border-white/[0.06]">
                    ...
                    {/* Pulse Layer */}
                    <div className="absolute inset-0 w-24 h-24 bg-slate-900/10 dark:bg-primary/20 rounded-3xl animate-ping opacity-20" />
                  </div>
                  
                  <span className="px-4 py-1.5 bg-slate-900 dark:bg-primary text-white rounded-full text-[10px] font-black uppercase tracking-[0.3em] mb-3 relative z-10 shadow-lg dark:shadow-primary/20">
                    Active Agent
                  </span>
                  <h3 className="text-2xl font-black text-slate-900 dark:text-white mb-2 relative z-10">{activeAgent}</h3>
                  <p className="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-widest relative z-10 line-clamp-2 px-4 leading-relaxed">
               ...
               <div className="mt-auto">
                 <div className="flex justify-between items-end mb-3 px-1">
                   <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Momentum</span>
                   <span className="text-xl font-black text-slate-900 dark:text-white tabular-nums">{progress}%</span>
                 </div>
            ...
            <div className="col-span-12 lg:col-span-7 flex flex-col bg-slate-50/30 dark:bg-white/[0.01] overflow-hidden">
              <div className="p-6 border-b border-slate-50 dark:border-white/[0.04] flex items-center gap-3 bg-white/50 dark:bg-transparent backdrop-blur-sm shrink-0">
                <span className="material-symbols-outlined text-sm text-slate-400 dark:text-slate-500">terminal</span>
                <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Autonomous Feed Stream</span>
              </div>
              
              <div 
                ref={scrollRef}
                className="flex-1 p-10 py-6 overflow-y-auto space-y-6 scroll-smooth"
              >
                {Array.isArray(logs) && logs.length > 0 ? (
                  logs.map((log) => (
                    <motion.div 
                      ...
                      key={log.id} 
                      className="flex gap-6 group border-l-2 border-slate-200 dark:border-white/[0.06] pl-6 hover:border-slate-400 dark:hover:border-primary/40 transition-all"
                    >
                  ...
                  <div className="h-full flex flex-col items-center justify-center opacity-30 grayscale">
                     ...
                     <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-600">Awaiting Neural Link...</p>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="px-10 py-8 border-t border-slate-50 dark:border-white/[0.04] bg-slate-50/50 dark:bg-white/[0.02] flex items-center justify-between">
             <div className="flex items-center gap-3 opacity-50 hover:opacity-100 transition-opacity">
                ...
                <p className="text-[10px] text-slate-500 dark:text-slate-600 font-black uppercase tracking-widest leading-none">
             ...
             <div className="flex gap-4">
                {onCancel && !hasError && (
                  <button
                    onClick={onCancel}
                    className="px-8 py-4 bg-white dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.06] text-slate-500 dark:text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 hover:border-rose-200 dark:hover:border-rose-400/20 hover:bg-rose-50 dark:hover:bg-rose-500/10 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all shadow-sm dark:shadow-none active:scale-95 flex items-center gap-2 group"
                  >
                ...
                {hasError && (
                  <button 
                    ...
                    className="px-10 py-4 rounded-2xl bg-slate-900 dark:bg-primary text-white text-[10px] font-black uppercase tracking-widest shadow-xl dark:shadow-primary/20 active:scale-95 transition-all"
                  >
                    Confirm Exit
                  </button>
                )}
